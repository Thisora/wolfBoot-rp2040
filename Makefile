...
CFLAGS+=-fomit-frame-pointer #Added to fix issue with cortex-m0 (issue 241)
...
LSCRIPT_FINAL:=../linker-script/complet.ld
...
ifeq ($(USE_GCC_HEADLESS),1)
  ...
  LDFLAGS_COMPLET+=-T $(LSCRIPT_FINAL) -Wl,-gc-sections -Wl,-Map=wolfboot.map -ffreestanding -nostartfiles
endif

#TODO REMOVE FACTORY:BIN --> OLD
# MAIN_TARGET=factory.bin
MAIN_TARGET=factory.elf
...
# #Added to get the correcte binary app
# ifeq ($(TARGET),pico)
# 	MAIN_TARGET:=wolfboot.elf tools/bin-assemble/bin-assemble ../build/image_v1_signed.bin internal_flash.dd
# endif
...
# No need anymore because we use custom image app 
...
../build/image_v1_signed.bin: $(BOOT_IMG)
	@echo "\t[SIGN] $(BOOT_IMG)"
...
internal_flash.dd: image_v1_signed.bin wolfboot.elf $(BINASSEMBLE)
	@echo "\t[MERGE] internal_flash.dd"
	$(Q)dd if=/dev/zero bs=1 count=$$(($(WOLFBOOT_SECTOR_SIZE))) > /tmp/swap
	$(Q)$(BINASSEMBLE) $@ 0 ../build/image_v1_signed.bin \
...
# factory.bin: $(BOOT_IMG) wolfboot.bin $(PRIVATE_KEY) image_v1_signed.bin $(BINASSEMBLE)
# 	@echo "\t[MERGE] $@"
...
clean:
	...
	@rm -f ../build/obj/*.o
	@rm -f ../build/image_v1_signed.bin
